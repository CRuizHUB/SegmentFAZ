...
def mascaraY():
    a=mascaraxreparada(45,8)
    b=mascaracruzreparada(45,8)
    b[:,0:17]=0
    b[:,29:45]=0
    Y=np.zeros_like(a)
    Y[0:22,:]=a[0:22,:]
...
def Segmentacioniterativa(names,masks):
    ...
    for i in range(len(names)):
        ...
        #Calculo areas para estadísticas
        areainterna=MascaraConvolucion.sum()/255
        areasinternas.append(areainterna)
        areaexterna=MascaraConvolucionExterna.sum()/255
        areasexternas.append(areaexterna)
        #Dibujo resultado
    ...
    return(areasinternas,areasexternas,errores,erroresexternos)
...
Areas,AreasExternas,Errores,ErroresExternos=Segmentacioniterativa(ImagenesOriginales,Mascaras) #Al nombrar en el argumento search_files, esta línea es la que borra los plexos 2 y 4.
plt.hist(Errores,bins=100)
plt.hist(ErroresExternos,bins=100)