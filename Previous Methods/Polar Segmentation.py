...
def detectaFAZ(img2,thresholdX,thresholdY): #Aquí, introducimos la imagen tras detectar bodes con CANNY en primer lugar, y luego los thresholds para las filas y columnas.
  ...
  for i in range(len(pixelesapintar)):
    for j in range(pixelesapintar[i].size):
      MascaraROI[i,int(pixelesapintar[i][j])]=255 #Pinto los píxeles que hay indicados en 'pixelesapintar' de blanco.
  ...
  zonaplanahorizontal=encuentrazonaplana(centros,thresholdX) #Esto lo que hace es, dado el array con la posicion de los centros, buscar el punto donde, si se grafica, se obtiene la zona plana, ya que serán las filas que conforman la FAZ.

  ######Repito el proceso con la transpuesta, para así, sacar las columnas#####
...
def segmenta(img): #Aqui ya solo hay que meter la imagen original. Lo hace todo él.
    ...
    ConvKernel=np.ones((KernSize,KernSize))/KernSize**2
...
img=cv2.imread("original.tif",0) #Leo la imagen
resultadofinal, perfiles, coordenadaspintadas=segmenta(img)
bordefinal=extract_perimeter(resultadofinal) #Extraigo solo el borde
cv2.imwrite("resultado.png",superponerFAZ(img, bordefinal, 0))
cv2.imwrite("prueba.png",superponerFAZ(img, detectaFAZ(detectabordes(img,17,19),0.15,0.15), 0))